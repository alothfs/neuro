...
def display_dashboard():
    ...
    if 'first_login' in st.session_state and st.session_state.first_login:
        st.markdown("""
        <div style="padding: 2rem; background-color: #f0f4f8; border-radius: 10px; margin-bottom: 1rem;">
            <h3>Welcome to Neuro Finance!</h3>
            <p>Let's set up your financial dashboard. Start by entering your current financial information.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.form("initial_setup"):
            ...
            with col1:
                balance = st.number_input("Current Account Balance (€)", min_value=0.0, value=0.0)

            with col2:
                savings = st.number_input("Savings (€)", min_value=0.0, value=0.0)

            with col3:
                investments = st.number_input("Investments (€)", min_value=0.0, value=0.0)

            submit_initial = st.form_submit_button("Save Financial Information")
    ...
    with col1:
        ...
        """.format(st.session_state.balance), unsafe_allow_html=True)
    ...
    with col2:
        ...
        """.format(st.session_state.savings), unsafe_allow_html=True)
    ...
    with col3:
        ...
        """.format(st.session_state.investments), unsafe_allow_html=True)

    # Update financial information
    with st.expander("Update Financial Information"):
        with st.form("update_finances"):
            col1, col2, col3 = st.columns(3)

            with col1:
                new_balance = st.number_input("Current Account Balance (€)", min_value=0.0, value=st.session_state.balance)

            with col2:
                new_savings = st.number_input("Savings (€)", min_value=0.0, value=st.session_state.savings)

            with col3:
                new_investments = st.number_input("Investments (€)", min_value=0.0, value=st.session_state.investments)
            ...
            if submit_update:
                ...
                st.session_state.savings = new_savings
                st.session_state.investments = new_investments
                st.success("Financial information updated!")
                st.rerun()
    ...
    with st.form("quick_transaction"):
        ...
        with col2:
            ...
            transaction_description = st.text_input("Description", placeholder="e.g., Lunch at Restaurant")
    ...
    if not st.session_state.transactions.empty:
        ...
        recent_transactions = st.session_state.transactions.copy()

        if not pd.api.types.is_datetime64_any_dtype(recent_transactions['date']):
            recent_transactions['date'] = pd.to_datetime(recent_transactions['date'])

        recent_transactions = recent_transactions.sort_values(by='date', ascending=False).head(5)
        ...
        st.info("No transactions yet. Add your first transaction using the form above.")
    ...
    if not st.session_state.transactions.empty:
        ...
        # Only show insights if we have both income and expenses
        if total_income > 0 and total_expenses > 0:
            savings_rate = ((total_income - total_expenses) / total_income) * 100

            # Group expenses by category
            expense_by_category = transactions_df[transactions_df['type'] == 'expense'].groupby('category')['amount'].sum().abs()
            top_expense = expense_by_category.idxmax() if not expense_by_category.empty else "N/A"
            top_expense_amount = expense_by_category.max() if not expense_by_category.empty else 0
            ...
            with col1:
                ...
                <div class="insights-card">
                    ...
                    <p>{'Great job!' if savings_rate >= 20 else 'Try to aim for at least 20%'}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                <div class="insights-card">
                    ...
                    <h2>{top_expense}</h2>
                    <p>€{top_expense_amount:.2f} ({(top_expense_amount/total_expenses*100):.1f}% of expenses)</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("Add both income and expense transactions to see financial insights.")
    else:
        st.info("Add transactions to see financial insights.")

    # Financial goals progress
    if st.session_state.goals:
        st.subheader("Financial Goals Progress")

        for goal in st.session_state.goals[:2]:  # Show only up to 2 goals on dashboard
            progress = (goal["current"] / goal["target"]) * 100
            ...
            <div style="padding: 1rem; background-color: #f0f4f8; border-radius: 10px; margin-bottom: 1rem;">
                <h4 style="margin-top: 0;">{goal["name"]}</h4>
                <div class="progress-container">
                    <div class="progress-bar" style="width: {progress}%; background-color: #6c5ce7;"></div>
                </div>
                <div style="display: flex; justify-content: space-between; margin-top: 0.5rem;">
                    <span>€{goal["current"]:.2f}</span>
                    <span>€{goal["target"]:.2f}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.info("No financial goals set. Set your first goal in the Goals section.")
...
def display_transactions():
    ...
    with st.form("add_transaction"):
        ...
        with col1:
            transaction_type = st.selectbox("Type", ["expense", "income"], key="trans_type")
            transaction_date = st.date_input("Date", value=datetime.now(), key="trans_date")
            transaction_category = st.selectbox(
                "Category",
                ["Income", "Groceries", "Dining", "Entertainment", "Transport", "Shopping", "Utilities", "Other"],
                key="trans_category"
        ...
        with col2:
            ...
            transaction_description = st.text_input("Description", placeholder="e.g., Lunch at Restaurant", key="trans_desc")
    ...
    tab1, tab2 = st.tabs(["List View", "Analysis"])
    ...
    with tab2:
        if not st.session_state.transactions.empty:
            ...
            try:
                ...
                st.info("Please make sure you have plotly installed: pip install plotly")
        else:
            st.info("Add transactions to see your cash flow analysis.")
